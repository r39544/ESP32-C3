...
### 云上报 (ThingSpeak)
- 配置页「云上报」填 ThingSpeak 通道的 **Write API Key** 与**通道号**,勾选启用后,设备每隔「上报间隔」(默认 60 秒,免费版 ≥15 秒)通过 HTTPS 把 **eCO₂ / TVOC / 温度 / 湿度 / AQI** 上传到 `api.thingspeak.com`。
- 字段映射:`field1`=eCO₂(ppm)、`field2`=TVOC(ppb)、`field3`=温度(°C)、`field4`=湿度(%)、`field5`=AQI。
- 上报同样受「启动后延迟记录」宽限控制(跳过预热),且需 WiFi 在线、传感器有效。
- ThingSpeak 免费版:300 万条/年、最小间隔 15 秒、数据长期保留,自带图表与 CSV 导出。
- ⚠️ 上报为**阻塞**操作(约 1-5 秒),期间网页刷新会短暂停顿,属正常;上报间隔建议 ≥60 秒。

### 历史数据记录(30 天环形)
- 每 **1 分钟**在 LittleFS 中追加一条 12 字节二进制记录(时间戳、eCO₂、TVOC、温度、湿度)。
- 按**日历日**存放:`/log/YYYYMMDD.bin`。
- **环形覆盖**:开机后 + 每小时清理,只保留最近 **30 天**,超过自动删除最旧的。
- 仅在 NTP 时间已同步、两个传感器均有有效读数、**且已过启动宽限**(默认 180 秒,可在配置页「启动后延迟记录」调整)后才记录——彻底跳过 ENS160 预热期(约 3 分钟)的垃圾数据。
